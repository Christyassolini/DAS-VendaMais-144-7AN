    ...
    try:
        ...
        for row in rows:
            dest_cursor.execute("""
                ...
                USING (VALUES (?, ?, ?)) AS s (id_produto, dt_referencia, qt_saldo)
                ON t.id_produto = s.id_produto AND t.dt_referencia = s.dt_referencia
                WHEN MATCHED THEN UPDATE SET t.qt_saldo = s.qt_saldo
                WHEN NOT MATCHED THEN INSERT (id_produto, dt_referencia, qt_saldo) VALUES (s.id_produto, s.dt_referencia, s.qt_saldo);
            """, row.id_produto, row.dt_referencia, row.qt_saldo)
        dest.commit()
